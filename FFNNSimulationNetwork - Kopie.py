...
#Modell definieren
model = Sequential()
model.add(Dense(units = 200, input_shape=(7,), activation='relu'))   #Eingangschicht
model.add(Dense(units = 200, activation='relu'))                #Mittelschichten
model.add(Dense(units=200, activation='relu'))
model.add(Dense(units=2))                                   #Ausgangsschicht
...
def create_model(trial):
    model = Sequential()
    model.add(Dense(units=trial.suggest_int('num_neurons', 50, 200), input_shape=(7,), activation='relu'))  # Eingangsschicht
...
def objective(trial):
    ...
    history = model.fit(train_inputs.reshape(-1, 4), train_outputs.reshape(-1, 1),
                        epochs=100, batch_size=1, callbacks=[checkpoint],
...
history = model.fit(train_inputs, train_outputs, epochs=100, batch_size=1, callbacks=callbacks_list, validation_data=(validate_inputs,validate_outputs), verbose=2)
#model.load_weights("\\Users\\Felix Koch\\Documents\\BachelorArbeit\\ModelsFFnn\\weights-09-0.0000012.hdf5")

model.save('FFNN_Einfachpendel.keras')
